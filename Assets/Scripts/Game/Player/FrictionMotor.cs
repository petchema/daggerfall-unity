            ...
            if (tryingToMoveForwards || tryingToMoveBackwards)
            {
                ...
                if (testMagnitude < Mathf.Pow(stuckMovementThreshold, 2))
                {
                    ...
                    if (stuckFrameCount > stuckFrameThreshold)
                    {
                        ...
                        // Vector3 displacement = sampleDirection * stuckSampleForward - Vector3.up * stuckSampleUpward;
                        ...
                        if (!Physics.SphereCast(new Ray(originTop, displacementUp), controller.radius, displacementUp.magnitude))
                        {
                            ...
                            if (!Physics.SphereCast(new Ray(originBottom, displacementForward), controller.radius, displacementForward.magnitude) &&
                                !Physics.SphereCast(new Ray(originTop, displacementForward), controller.radius, displacementForward.magnitude))
                            {
                                ...
                                RaycastHit hit;
                                if (!Physics.SphereCast(new Ray(originBottom, displacementDownward), controller.radius, out hit, displacementDownward.magnitude))
                                {
